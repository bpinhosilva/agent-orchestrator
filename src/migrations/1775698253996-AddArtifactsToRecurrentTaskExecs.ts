...
export class AddArtifactsToRecurrentTaskExecs1775698253996 implements MigrationInterface {
  name = 'AddArtifactsToRecurrentTaskExecs1775698253996';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!isSqlite) {
      await queryRunner.query(
        `ALTER TABLE "recurrent_task_execs" ADD COLUMN IF NOT EXISTS "artifacts" text`,
      );
      return;
    }
    ...
    await this.execute(
      queryRunner,
      `DROP INDEX "IDX_f5bb7be33ddb87ac0f04807b4a"`,
    );
    await this.execute(
      queryRunner,
      `CREATE TABLE "temporary_recurrent_task_execs" ("id" varchar PRIMARY KEY NOT NULL, "status" varchar CHECK( "status" IN ('running','success','failure','canceled') ) NOT NULL DEFAULT ('running'), "result" text, "latencyMs" integer, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "recurrentTaskId" varchar NOT NULL, "artifacts" text, CONSTRAINT "FK_f5bb7be33ddb87ac0f04807b4ab" FOREIGN KEY ("recurrentTaskId") REFERENCES "recurrent_tasks" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`,
    );
    await this.execute(
      queryRunner,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!isSqlite) {
      await queryRunner.query(
        `ALTER TABLE "recurrent_task_execs" DROP COLUMN IF EXISTS "artifacts"`,
      );
      return;
    }
    ...
    await this.execute(
      queryRunner,
      `DROP TABLE "temporary_recurrent_task_execs"`,
    );
    await this.execute(
      queryRunner,
      `CREATE INDEX "IDX_f5bb7be33ddb87ac0f04807b4a" ON "recurrent_task_execs" ("recurrentTaskId") `,
